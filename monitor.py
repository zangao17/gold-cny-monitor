...
def is_sge_trading_session(now):
    if now.weekday() >= 5:
        return False
    current_time = now.time()
    return datetime.strptime("09:00", "%H:%M").time() <= current_time <= datetime.strptime(
        "15:30", "%H:%M"
    ).time()


def is_fresh_sge_quote(quote, now):
    try:
        market_date = datetime.strptime(quote["market_time"], "%Y-%m-%d").date()
    except (TypeError, ValueError):
        return False
    return market_date == now.date()


def fetch_quote(now):
    if is_sge_trading_session(now):
        try:
            quote = fetch_sge_price()
            if is_fresh_sge_quote(quote, now):
                return quote
            print("SGE quote is stale; using estimated fallback", file=sys.stderr)
        except Exception as error:
            print(f"SGE fetch failed; using estimated fallback: {error}", file=sys.stderr)

    quote = fetch_estimated_price()
    quote["instrument"] = "\u5de5\u884c\u79ef\u5b58\u91d1\u53c2\u8003\u4f30\u503c"
    quote["source_kind"] = "international_estimate"
    quote["source"] = "\u56fd\u9645\u73b0\u8d27\u9ec4\u91d1\u4e0e\u7f8e\u5143/\u4eba\u6c11\u5e01\u6c47\u7387\u6298\u7b97"
    quote["session_note"] = (
        "\u4e0a\u91d1\u6240\u65e5\u76d8\u5df2\u4f11\u5e02\uff1b\u5f53\u524d\u4e3a\u56fd\u9645\u91d1\u4ef7\u6298\u7b97\u53c2\u8003\uff0c"
        "\u4e0d\u662f\u5de5\u884c\u53ef\u6210\u4ea4\u62a5\u4ef7\u3002"
    )
    return quote
...
def build_email_html(quote, change, change_ratio, timestamp, portfolio_report, reasons, news):
    ...
    session_note_html = ""
    if quote.get("session_note"):
        session_note_html = (
            "<p style=\"margin:12px 0 0;padding:10px 12px;background:#fff9e8;"
            "border-left:3px solid #c99a2e;color:#665319;font-size:12px;line-height:1.6;\">"
            f"{escape(quote['session_note'])}</p>"
        )
